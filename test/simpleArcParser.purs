...
import Perspectives.Parsing.Arc.AST (ContextE(..), ContextPart(..), MandatoryAttribute(..), PropertyE(..), PropertyPart(..), RoleE(..), RolePart(..), ViewE(..))
import Perspectives.Parsing.Arc.IndentParser (runIndentParser)
import Perspectives.Parsing.Arc.Simple (domain, propertyE, roleE, viewE)
...
theSuite = suite "Perspectives.Parsing.Arc.Simple" do
  ...
  test "Role that is mandatory" do
    ...
    case r of
      ...
      (Right ctxt@(RE (RoleE{roleParts}))) -> case head roleParts of
        ...
        (Just (MA (MandatoryAttribute b))) -> assert "The Role should have the attribute 'Mandatory' with value 'True'" b
  ...
  test "Property with functional and mandatory properties" do
    (r :: Either ParseError RolePart) <- runP $ runIndentParser "Property : NumberProperty : MyProperty\n  NonFunctional : True\n  Mandatory : False" propertyE
    case r of
      (Left e) -> assert (show e) false
      (Right ctxt@(PE (PropertyE{propertyParts}))) -> do
