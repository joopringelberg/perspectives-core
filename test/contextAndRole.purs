...
import Perspectives.Instances (getPerspectContext, getPerspectRol)
...
theSuite = suiteSkip "ContextAndRole" do
  ...
  test "Role assignments" $ runP do
      ...
      context1 <- getPerspectContext (ContextInstance "model:User$MyTestCase") >>=
        (pure <<< \c -> addContext_rolInContext c (EnumeratedRoleType "model:Test$TestCase$SomeRole") (RoleInstance "blabla"))
      liftAff $ assert "addContext_rolInContext fails" (length (context_rolInContext context1 (EnumeratedRoleType "model:Test$TestCase$SomeRole")) == 1)

      context2 <- pure $ removeContext_rolInContext context1 (EnumeratedRoleType "model:Test$TestCase$SomeRole") (RoleInstance "blabla")
      liftAff $ assert "removeContext_rolInContext fails" (length (context_rolInContext context2 (EnumeratedRoleType "model:Test$TestCase$SomeRole")) == 0)

      context3 <- pure $ setContext_rolInContext context2 (EnumeratedRoleType "model:Test$TestCase$SomeRole") [(RoleInstance "blabla")]
      liftAff $ assert "setContext_rolInContext fails" (length (context_rolInContext context3 (EnumeratedRoleType "model:Test$TestCase$SomeRole")) == 1)

      context4 <- pure $ deleteContext_rolInContext context3 (EnumeratedRoleType "model:Test$TestCase$SomeRole")
      liftAff $ assert "deleteContext_rolInContext fails" (length (context_rolInContext context4 (EnumeratedRoleType "model:Test$TestCase$SomeRole")) == 0)

      context5 <- pure $ modifyContext_rolInContext context4 (EnumeratedRoleType "model:Test$TestCase$SomeRole") (const [(RoleInstance "blabla")])
      liftAff $ assert "modifyContext_rolInContext fails" (length (context_rolInContext context5 (EnumeratedRoleType "model:Test$TestCase$SomeRole")) == 1)
  ...
  test "Property assignments" $ runP do
      ...
      role1 <- getPerspectRol (RoleInstance "model:User$MyTestCase$Self_0001") >>=
        (pure <<< \c -> addRol_property c (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1") [Value "1", Value "2"])
      liftAff $ assert "addRol_property fails" (length (rol_property role1 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1")) == 2)

      role2 <- pure $ removeRol_property role1 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1") [Value "2"]
      liftAff $ assert "removeRol_property fails" (length (rol_property role2 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1")) == 1)

      role3 <- pure $ deleteRol_property role2 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1")
      liftAff $ assert "deleteRol_property fails" (length (rol_property role3 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1")) == 0)

      role4 <- pure $ setRol_property role3 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1") [Value "1", Value "2"]
      liftAff $ assert "setRol_property fails" (length (rol_property role4 (EnumeratedPropertyType "model:Test$TestCase$Self$Prop1")) == 2)
      ...
      role7 <- pure $ addRol_gevuldeRollen role6 (EnumeratedRoleType "model:Test$TestCase$SomeRole") (RoleInstance "blabla")
      liftAff $ assert "addRol_gevuldeRollen fails" ((rol_gevuldeRol role7 (EnumeratedRoleType "model:Test$TestCase$SomeRole") == [(RoleInstance "blabla")]))

      role8 <- pure $ removeRol_gevuldeRollen role7 (EnumeratedRoleType "model:Test$TestCase$SomeRole") (RoleInstance "blabla")
      liftAff $ assert "removeRol_gevuldeRollen fails" ((rol_gevuldeRol role8 (EnumeratedRoleType "model:Test$TestCase$SomeRole") == []))
