...
compileExpressions = do
  ...
  where
    ...
    setAffectedContextCalculations qfd = for_ (invertFunctionDescription qfd) setPath

      where
        setPath :: QueryFunctionDescription -> PhaseThree Unit
        setPath path = do
          case path of
            (BQD _ (QF.BinaryCombinator QF.ComposeF) (SQD dom qf _ _ _) _ _ _ _) -> case qf of
            ...
            _ -> throwError $ Custom "The results of invertFunctionDescription should be compositions."

          case path of
            (BQD _ _ _ subPath@(BQD _ (QF.BinaryCombinator QF.ComposeF) _ _ _ _ _) _ _ _) -> setPath subPath
            _ -> pure unit
