...
executeInTopologicalOrder :: forall m item label. Monad m => Eq label => 
  (item -> label) -> 
  (item -> Array label) ->
  ToSort item ->
  (item -> m item) ->
...
sortTopologically  :: forall m item. Monad m => Eq item =>
  (item -> Array item) ->
  ToSort item ->
  m (Array item)
...
sortTopologically_  :: forall m item label. Monad m => Eq label =>
  (item -> label) ->
  (item -> Array label) ->
  ToSort item ->
  m (Array item)
